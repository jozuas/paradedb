...
SELECT issue2659_base.*,
       any_value(issue2659_text.full_text) as any_full_text
FROM issue2659_base
...
-- error
SELECT * FROM issue2659_view WHERE any_full_text @@@ 'document';


explain SELECT issue2659_base.*, any_value(issue2659_text.full_text)
FROM issue2659_base
         LEFT JOIN issue2659_text ON issue2659_text.doc_id = issue2659_base.id
WHERE full_text = 'document'
GROUP BY issue2659_base.id;
